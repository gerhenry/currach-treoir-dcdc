* TR100 CONTROL V2 (STABLE SOFTSTART + PWM)
...
* Controlled charge ONLY when below clamp
BSSCHG 0 ss I = { (V(ss)<VREF_FINAL) ? abs(V(ibias_ss)) : 0 }

* Hard clamp (prevents runaway)
VCLAMP ss_clamp 0 {VREF_FINAL}
ECLAMP ss 0 VALUE = { min(V(ss), V(ss_clamp)) }
...
wrdata ~/tmp/tr100_ctrl_bias_ss_pwm_v2.dat v(enable) v(ibias_ss) v(ibias_ramp) v(ss) v(vref_ss) v(ramp) v(pwm)
